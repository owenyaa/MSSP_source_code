...
residual(N_dof+4,:)=sum(vector_w(:,1).*Harm_parameter_a(:,2));
residual(N_dof+5,:)=sum(vector_w(:,2).*Harm_parameter_a(:,4));
...
for j=1:N_w0
    ...
    residual(N_dof1*j+1:(1+j)*N_dof1-5,:)=M*ddx_w0+K*x_w0+3*k22*[(x(1,:)-x(2,:)).^2.*(x_w0(1,:)-x_w0(2,:));(x(2,:)-x(1,:)).^2.*(x_w0(2,:)-x_w0(1,:))];
    residual((1+j)*N_dof1-4,:)=m1*dx(1,:).*dx_w0(1,:)+m2*dx(2,:).*dx_w0(2,:)+k1*x(1,:).*x_w0(1,:)+k2*x(2,:).*x_w0(2,:)+k12*(x(1,:)-x(2,:)).*(x_w0(1,:)-x_w0(2,:))+k22*(x(1,:)-x(2,:)).^3.*(x_w0(1,:)-x_w0(2,:));
    residual((1+j)*N_dof1-3,:)=0;
    residual((1+j)*N_dof1-2,:)=0;
    residual((1+j)*N_dof1-1,:)=sum(index_global(:,j).*Harm_parameter_a(:,2));
    residual((1+j)*N_dof1,:)=sum(index_global(:,j+2).*Harm_parameter_a(:,4));
end
% 计算谐波系数的灵敏度
for i=1:2*N_harm*N_dof
    ...
    residual(N_dof1*(i+N_w0)+1:N_dof1*(i+N_w0+1)-5,:)=M*ddx_a+K*x_a+3*k22*[(x(1,:)-x(2,:)).^2.*(x_a(1,:)-x_a(2,:));(x(2,:)-x(1,:)).^2.*(x_a(2,:)-x_a(1,:))];
    residual(N_dof1*(i+N_w0+1)-4,:)=m1*dx(1,:).*dx_a(1,:)+m2*dx(2,:).*dx_a(2,:)+k1*x(1,:).*x_a(1,:)+k2*x(2,:).*x_a(2,:)+k12*(x(1,:)-x(2,:)).*(x_a(1,:)-x_a(2,:))+k22*(x(1,:)-x(2,:)).^3.*(x_a(1,:)-x_a(2,:));
    if i<N_harm*N_dof
        residual(N_dof1*(i+N_w0+1)-3,:)=mod(i,2);
        residual(N_dof1*(i+N_w0+1)-2,:)=0;
    else
        residual(N_dof1*(i+N_w0+1)-3,:)=0;
        residual(N_dof1*(i+N_w0+1)-2,:)=mod(i,2);
    end
    if mod(i,2)==1
        residual(N_dof1*(i+N_w0+1)-1,:)=0;
        residual(N_dof1*(i+N_w0+1),:)=0;
    else
        residual(N_dof1*(i+N_w0+1)-1,:)=sum(sum(sensitivity_parameter_a(:,1:2).*[vector_w(:,1),vector_w(:,1)]));
        residual(N_dof1*(i+N_w0+1),:)=sum(sum(sensitivity_parameter_a(:,3:4).*[vector_w(:,2),vector_w(:,2)]));
    end
